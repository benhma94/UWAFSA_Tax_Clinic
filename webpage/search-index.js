// Static search index for the UW AFSA Tax Clinic public website.
// Each entry: { page, url, section, text }
// Loaded by search.html before filtering.

var SEARCH_INDEX = [

  // ── Eligibility and Details (index.html) ──────────────────────────────────
  {
    page: "Eligibility and Details",
    url: "index.html",
    section: "Clinic Dates 2026",
    text: "Tax Clinic 2026 dates: Saturday March 21 at STC 1012 (10:00 AM – 7:30 PM), Sunday March 22 at STC 1012, Saturday March 28 at RCH 301, Sunday March 29 at STC 1012. Walk-in basis, no appointment needed for most clients."
  },
  {
    page: "Eligibility and Details",
    url: "index.html",
    section: "Eligibility — Who We Cannot Help",
    text: "We cannot prepare your return if it involves: self-employed income (including Uber, DoorDash, independent contractors), rental income (excluding sublet of a property you rent), interest income over $1,200 (except T2202 tuition credit claimants), foreign investment property worth more than $100,000, capital gains or losses from selling shares or securities, employment expenses (T2200), bankruptcy, deceased persons."
  },
  {
    page: "Eligibility and Details",
    url: "index.html",
    section: "Income Limits",
    text: "Income thresholds: Individual $40,000, Couple $55,000, each additional dependant +$5,000. Clients claiming T2202 tuition credits are welcome regardless of total income or interest income. Other restrictions still apply."
  },
  {
    page: "Eligibility and Details",
    url: "index.html",
    section: "Appointment Booking",
    text: "No appointment is required for most clients. If your situation is slightly complex and may take more time, use our screening tool to determine if you need to book an appointment. Priority appointments are available for complex cases."
  },
  {
    page: "Eligibility and Details",
    url: "index.html",
    section: "International Students",
    text: "International students are welcome and encouraged to file a tax return even with no income. We will help you get an Individual Tax Number (ITN) if you are not eligible for a Social Insurance Number (SIN). We cannot prepare tax returns for other countries (e.g. US returns), but if you bring a completed foreign tax return we may be able to complete the Canadian one."
  },
  {
    page: "Eligibility and Details",
    url: "index.html",
    section: "What to Bring — All Clients",
    text: "All clients should bring: government-issued photo ID (required), Social Insurance Number (SIN) or Individual Tax Number (ITN), USB drive (if available), information slips (T3, T4, T4A, T5, T5013, etc.), 2024 Notice of Assessment (if not filing for the first time). A printable checklist is available on the website."
  },
  {
    page: "Eligibility and Details",
    url: "index.html",
    section: "What to Bring — Students",
    text: "Students should also bring: tuition slip (T2202) from Quest, T4 employment slip, T4A for scholarships/RESP withdrawals/CESB/CERB, T4E employment insurance, rent receipts, rent or property tax paid in Ontario. Access T2202 on Quest → Finances → Tax Receipts. Access T4/T4A from UW on Workday → Main Menu → Personal → Documents."
  },
  {
    page: "Eligibility and Details",
    url: "index.html",
    section: "What to Bring — Less Common Items",
    text: "Less common documents: foreign income, medical expenses, moving expenses, RRSP contribution slips, FHSA contribution slips, student loan interest, charitable donations, T3/T5/T5007, tips or gratuities earned, carrying charges (investment fees), Home Buyer's Amount if you purchased a new home in 2025."
  },
  {
    page: "Eligibility and Details",
    url: "index.html",
    section: "What Not to Bring",
    text: "Do not bring: home office expenses, pay stubs, GST/HST notices of assessment."
  },
  {
    page: "Eligibility and Details",
    url: "index.html",
    section: "Rent Receipts",
    text: "Rent receipts should include: address lived at (with postal code), number of months at that address, total rent paid in 2025, name of landlord or person you subletted from. A letter written yourself and signed by your landlord is acceptable. Students in Ontario university residence may qualify for a $25 credit without a receipt."
  },
  {
    page: "Eligibility and Details",
    url: "index.html",
    section: "Moving Expenses",
    text: "If you moved 40 km closer to a new workplace, you may deduct moving expenses. Bring: Google Maps distance from old residence to new work and new residence to new work, addresses of both residences, date of move, date you started the new job, employer name and address, kilometres between residences for the simplified method, meals eaten during move, number of nights."
  },
  {
    page: "Eligibility and Details",
    url: "index.html",
    section: "Charitable Donations — UW Endowment Fund",
    text: "All UW students contribute to endowment fund(s) as part of student fees unless they opted out. Access donation receipts on Quest → Finances → Tax Receipts → Scroll down → PRINT."
  },
  {
    page: "Eligibility and Details",
    url: "index.html",
    section: "Health and Dental Plan Premiums",
    text: "Health and dental plan premiums are a medical expense. Contact the health/dental office for a receipt or letter stating the total premiums paid for 2025. Medical expenses may be transferable to parents depending on the situation. Find student health plan information at studentcare.ca."
  },

  // ── FAQ ───────────────────────────────────────────────────────────────────
  {
    page: "FAQ",
    url: "FAQ.html",
    section: "Walk-in or Appointment?",
    text: "No appointment is necessary. Walk-in services offer the greatest flexibility. If your situation is slightly complex, use the screening tool to determine if you need a priority appointment."
  },
  {
    page: "FAQ",
    url: "FAQ.html",
    section: "Online or Drop-Off Services",
    text: "Due to resource constraints, online filing and drop-off services are not available. We are constantly evaluating our service offering and this may be available in the future."
  },
  {
    page: "FAQ",
    url: "FAQ.html",
    section: "Review of Already Prepared Return",
    text: "Reviewing a tax return you have already prepared is not a service provided at the Tax Clinic."
  },
  {
    page: "FAQ",
    url: "FAQ.html",
    section: "Non-UW Students Welcome",
    text: "We do not restrict services to University of Waterloo students. We are open to everyone who meets our eligibility requirements."
  },
  {
    page: "FAQ",
    url: "FAQ.html",
    section: "How Long Does Filing Take?",
    text: "Generally 30–45 minutes to prepare your return. More complicated situations take longer. Having all documents organized speeds up the process significantly."
  },
  {
    page: "FAQ",
    url: "FAQ.html",
    section: "TD1 / TD1ON Form",
    text: "The TD1 and TD1ON forms determine the appropriate tax withholding from your paycheque. For most students, include only the basic personal amounts and tuition fees. If you have a lot of tuition credits your employer may not need to withhold any income tax."
  },
  {
    page: "FAQ",
    url: "FAQ.html",
    section: "Digital Documents",
    text: "Digital documents are fine. Download everything onto a USB drive or folder in advance to avoid searching during your appointment."
  },
  {
    page: "FAQ",
    url: "FAQ.html",
    section: "Do I Have to File a Tax Return?",
    text: "You are only required to file if the CRA requests it or you owe money. Filing is generally in your best interest: you may receive a refund from excess withholdings, low-income benefits like Ontario Trillium Benefit or GST/HST Credit (up to $1,200), carry-forward tuition credits (reducing future tax by $0.15 per $1 of tuition), and access to income-tested programs like OESP, provincial grants, mortgages."
  },
  {
    page: "FAQ",
    url: "FAQ.html",
    section: "Tuition Slip / T2202",
    text: "We cannot provide tuition slips (T2202) or T2200 forms as we are not representatives of the University of Waterloo. Contact the school's student financial services for tax documents."
  },
  {
    page: "FAQ",
    url: "FAQ.html",
    section: "Missing Documents",
    text: "All documents must be available at the time of filing. We cannot file a return if you know you are missing documents. It is also not possible to partially file a return."
  },
  {
    page: "FAQ",
    url: "FAQ.html",
    section: "T4 vs Pay Stub",
    text: "A pay stub is not a sufficient replacement for a T4. Your T4 may include information your pay stub does not, such as taxable benefits. You must have your T4 if employed."
  },
  {
    page: "FAQ",
    url: "FAQ.html",
    section: "Missing Rent Receipts",
    text: "If you know exactly how much rent you paid, we can input the amount. However, if CRA requests documentation on review you must be able to provide it. A rent receipt sample is available on the Post Tax Return Filing page."
  },
  {
    page: "FAQ",
    url: "FAQ.html",
    section: "Uber / DoorDash / Self-Employment",
    text: "Uber, DoorDash, and other gig work is considered self-employment income for tax purposes. We are unable to assist with returns that include self-employment income."
  },

  // ── Post Filing ───────────────────────────────────────────────────────────
  {
    page: "Post Tax Return Filing",
    url: "PostFiling.html",
    section: "After You Filed — What to Do",
    text: "If your return was E-Filed successfully, wait for CRA to assess it. Simple returns are usually assessed within 2–3 weeks; refunds are deposited directly or mailed by cheque. Processing times can be up to 6 months. If your return needs to be paper filed, mail the documents to the CRA at the applicable address. Register for CRA My Account."
  },
  {
    page: "Post Tax Return Filing",
    url: "PostFiling.html",
    section: "CRA My Account",
    text: "We strongly recommend creating a CRA My Account after your return has been assessed (a Notice of Assessment is required to create one). You can update your address, see CRA mail, update direct deposit, view prior year returns and current year tax slips, and check return status. Instructions are available as a document on this page."
  },
  {
    page: "Post Tax Return Filing",
    url: "PostFiling.html",
    section: "Ontario Electricity Support Program (OESP)",
    text: "Eligibility for the Ontario Electricity Support Program is based on your income from your tax return. If you and all adults in your household have filed, you may be eligible for a credit on your hydro bill, reducing monthly expenses."
  },
  {
    page: "Post Tax Return Filing",
    url: "PostFiling.html",
    section: "CRA Letter / Review Request",
    text: "Receiving a CRA letter asking for information is normal. Canada's tax system is honour-based and CRA may request documentation for claims they cannot verify automatically (e.g. rent receipts, income before coming to Canada). Read the letter carefully, provide the requested information by mail to the address given, and do not panic."
  },
  {
    page: "Post Tax Return Filing",
    url: "PostFiling.html",
    section: "Rent Receipt Requests from CRA",
    text: "If CRA reviews your rent claim, a sample rent receipt is provided on this page. It includes the typical CRA review letter and what is generally considered an acceptable receipt. As long as it includes all required information CRA will accept it."
  },
  {
    page: "Post Tax Return Filing",
    url: "PostFiling.html",
    section: "World Income Verification",
    text: "Newcomers to Canada may receive a world income verification request from CRA, which can pause benefits. To restore benefits: complete Form RC151 (detailing world income before entry to Canada), send a copy to CRA with a cover letter requesting benefit restoration. Response times vary from three weeks to several months."
  },
  {
    page: "Post Tax Return Filing",
    url: "PostFiling.html",
    section: "CRA Direct Deposit",
    text: "Direct deposit allows your tax refund and government benefits (GST/HST Credit, Ontario Trillium Benefit) to be deposited directly into your bank account. As of March 24, 2025, CRA no longer accepts direct deposit applications through your tax return. Set it up through your bank's online banking. Supported banks: CIBC, Simplii, National Bank, BMO, Scotiabank, TD, Tangerine, RBC."
  },

  // ── About Us ──────────────────────────────────────────────────────────────
  {
    page: "About Us",
    url: "about.html",
    section: "About the Tax Clinic",
    text: "The UW AFSA Tax Clinic is a student-run clinic under the Accounting & Finance Student Association (AFSA) Internal Services division. Operating since 2009, we have grown from a small group of students to a team of over 150 volunteers who help file taxes for over 1,000 residents of the KW and UW community every year."
  },
  {
    page: "About Us",
    url: "about.html",
    section: "Management Team",
    text: "The management team includes current students from the School of Accounting & Finance, a PhD student with a focus in Taxation, several CPAs, former Big 4 senior managers, a CFA Charterholder, a CFP Certificant, individuals on the Honour Roll for the CFE, and an Ontario Gold Medalist on the CFE."
  },
  {
    page: "About Us",
    url: "about.html",
    section: "Statistics — Returns Filed",
    text: "Returns filed by year: 2019 (1,281), 2022 (1,191), 2023 (1,142), 2024 (1,364), 2025 (1,011). Total across five seasons shown: 5,989 returns filed."
  },

  // ── Volunteer Applications ─────────────────────────────────────────────────
  {
    page: "Volunteer Applications",
    url: "volunteerapplications.html",
    section: "How to Apply",
    text: "Apply to volunteer at the UW AFSA Tax Clinic by filling out the application form. Three roles available: Filer (prepares tax returns), Frontline (receptionist), and Mentor (experienced filer who mentors first-timers). Applications open in fall each year."
  },
  {
    page: "Volunteer Applications",
    url: "volunteerapplications.html",
    section: "Filer Role Requirements",
    text: "Filer requirements: must be over 18, able to attend a mandatory training session in February, available to volunteer in person on clinic dates (March), and have a Windows PC for UFile software. International students who arrived in Canada in 2026 or later are not eligible for an EFILE number and cannot apply as a Filer."
  },
  {
    page: "Volunteer Applications",
    url: "volunteerapplications.html",
    section: "Filer Commitment",
    text: "Filer volunteers must: register for the CRA CVITP program and obtain an EFILE number, attend an 8-hour training session (February 28), complete a case-based test, and volunteer for at least 3 shifts (12 hours) across the 4 clinic dates. The Tax Clinic is an official CRA Community Volunteer Income Tax Program (CVITP) clinic."
  }
];
